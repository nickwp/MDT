...
string fInFileName = "";
string fOutFileName = "";
float fEffAtCosTheta0 = 0.45;
float fEffAtCosTheta1 = 0.55;
...
int main(int argc, char **argv)
{
    ...
    MDT->RegisterPMTType(fPMTType[0], new ResponseBoxandLine20inchHQELinear(fEffAtCosTheta0, fEffAtCosTheta1));
    MDT->RegisterPMTType(fPMTType[1], new Response3inchR14374Flat((fEffAtCosTheta0+fEffAtCosTheta1)*0.5));
...
bool ParseCmdArguments(int argc, char **argv)
{
	...
	for(int i=1; i<argc; i++)
	{
        ...
        else if( string(argv[i])=="-e" )		        { fEffAtCosTheta0 = atof( argv[i+1] ); i++;}
        else if( string(argv[i])=="-E" )		        { fEffAtCosTheta1 = atof( argv[i+1] ); i++;}
		else
		{
            cout<<" i: " << argv[i] <<endl;
			return false;
